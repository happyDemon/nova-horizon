    ...
    <card class="nova-horizon flex flex-col">
        <div :class="darkModeClass()">
            <nova-horizon-card-header class="p-3">
                <span v-if="$attrs.card.horizon && $attrs.card.horizon.name">{{$attrs.card.horizon.name}} -</span> Completed Jobs
...
export default {
    ...
    methods: {
        ...
        loadJobs(starting = -1, refreshing = false) {
            ...
            this.getHorizonRequest('api/jobs/completed?starting_at=' + starting + '&limit=' + this.perPage)
                .then(response => {
                    if (
                        refreshing &&
                        this.jobs.length &&
